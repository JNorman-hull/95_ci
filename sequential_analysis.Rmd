 ...
 create_mortality_table<- function(
  ...
  do.call(rbind, lapply(sample_sizes, function(n) {
    ...
    decision <- case_when(
      ...
      ci_upper_surv < desired_surv ~ "Reject H0",
      TRUE ~ "Continue"
    )
...
```{r create_decision_plot, echo= FALSE, include = FALSE}
...
create_decision_plot <- function(mortality_table) {
  ...
  y_limits <- c(0, y_max * 1.05)  # Add 5% padding
  
  # Create plot
  mortality_table %>%
    filter(sample_size > 0) %>%
    ggplot(aes(x = sample_size, y = n_mortality, color = decision)) +
    geom_point(size = 2, alpha = 0.8) +
    scale_color_manual(
      values = c("Accept H0" = "green", "Reject H0" = "red", "Continue" = "grey50"),
    ...
    labs(
      title = "Experimental Design Decision Boundaries",
      subtitle = "Accept (meets both criteria) | Reject (upper bound < target) | Continue (neither)",
      x = "Sample Size",
      y = "Observed Mortality Count",
      color = "Decision",
      fill = "Decision"
    ) +
    theme_JN()
...
-   Our first sample occurs at 100, which has already significantly reduced our likelihood of committing a type I error. We simple maintain our artifical alpha spend linearly throughout the experiment. Bare in mind there is no true alpha when we are just examining within group probability and want a fixed precision. That will come in the next approach of comparing impact to control groups.
